...
describe('strategic actions', () => {
  ...
  it('R5: the Technology primary can research a 2nd technology for 6 resources', () => {
    const s = withPlayer(withCards(withCards(toActionPhase(), 0, ['technology']), 1, []), 0, { tradeGoods: 1 })
    renderWithSession(s, <BoardScreen />)
    playCard('technology')
    fireEvent.click(screen.getByTestId('tech-card-antimass_deflectors'))
    const chk = screen.getByTestId('chk-second-tech')
    fireEvent.click(chk)
    expect(screen.getByTestId('tab-tech-1')).toBeTruthy()
    expect(screen.getByTestId('tab-tech-2')).toBeTruthy()
    fireEvent.click(screen.getByTestId('tech-card-gravity_drive'))
    expect(screen.getByTestId('btn-strategic-confirm').hasAttribute('disabled')).toBe(false)
    fireEvent.click(screen.getByTestId('btn-strategic-confirm'))
    fireEvent.click(screen.getByTestId('btn-secondary-decline'))
    fireEvent.click(screen.getByTestId('tab-side-0'))
    expect(screen.getByTestId('tech-0-antimass_deflectors')).toBeTruthy()
    expect(screen.getByTestId('tech-0-gravity_drive')).toBeTruthy()
  })
})
