...
describe('the board', () => {
  ...
  it('stacks the units of a system with a count badge', () => {
    ...
    expect(screen.getByTestId('sprite-home-n-0-dreadnought').getAttribute('width')).toBe('63')
  ...
  it('shows ground forces, structures and control tokens on the planets', () => {
    ...
    expect(screen.getByTestId('control-000').parentElement?.className).toBe('ctl-wrapper')
    expect(screen.getByTestId('control-000').parentElement?.getAttribute('title')).toContain('Controlled by A')
    expect(screen.getByTestId('control-000').parentElement?.querySelector('.ctl-sigil')?.getAttribute('src')).toBe('/assets/factions/l1z1x.png')
  ...
  it('shows a played command token per seat with a token on the system, and none on an idle system', () => {
    ...
    expect(seat0.parentElement?.className).toBe('act-wrapper')
    expect(seat0.parentElement?.getAttribute('title')).toContain('Activated by A')
    expect(seat0.parentElement?.querySelector('.act-sigil')?.getAttribute('src')).toBe('/assets/factions/l1z1x.png')
