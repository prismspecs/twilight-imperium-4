...
/**
 * R6 (lrr-factions.md, Star Forge): "Spend 1 strategy token to place 2 fighters or 1 destroyer from
 * reinforcements in a system containing one of your war suns." This is a direct placement, never a
 * production action (no resource cost, no space dock, no production step). LRR 683.3: a component action
 * cannot be performed if its ability cannot be completely resolved.
 *
 * Returns the war-sun system (if any) that can legally receive `unitType`: it holds a seat war sun and the
 * placement does not exceed capacity / the fleet pool (LRR 1123.3). Both the reinforcement count and the
 * fleet-pool / capacity probe are checked so the move is only offered when it can fully resolve.
 */
function starForgeTarget(state: GameState, seat: Seat, unitType: 'fighter' | 'destroyer'): string | undefined {
  const player = state.players[seat]
  const need = unitType === 'fighter' ? 2 : 1
  if ((player.reinforcements[unitType] ?? 0) < need) return undefined
  for (const [sysId, sys] of Object.entries(state.systems)) {
    if (!sys.space.some(u => u.owner === seat && u.type === 'warsun')) continue
    const probe: Unit[] = Array.from({ length: need }, (): Unit => ({ id: -1, type: unitType, owner: seat, damaged: false }))
    const nextSys = { ...sys, space: [...sys.space, ...probe] }
    if (checkFleet({ ...state, systems: { ...state.systems, [sysId]: nextSys } }, seat, sysId).ok) return sysId
  }
  return undefined
}

/** R6: Star Forge is offered when Muaat holds a strategy token and can place at least one unit type. */
export function canStarForge(state: GameState, seat: Seat): boolean {
  const player = state.players[seat]
  return player.faction === 'muaat' && player.tokens.strategy >= 1
    && (starForgeTarget(state, seat, 'fighter') !== undefined || starForgeTarget(state, seat, 'destroyer') !== undefined)
}

/** R6: the specific unit placement (2 fighters or 1 destroyer) can be completely resolved right now. */
export function canStarForgeUnit(state: GameState, seat: Seat, unitType: 'fighter' | 'destroyer'): boolean {
  const player = state.players[seat]
  return player.faction === 'muaat' && player.tokens.strategy >= 1 && starForgeTarget(state, seat, unitType) !== undefined
}

export function starForge(state: GameState, seat: Seat, unitType: 'fighter' | 'destroyer'): Result<GameState> {
  const ready = actionReady(state)
  if (!ready.ok) return ready
  if (!canStarForgeUnit(state, seat, unitType)) return { ok: false, error: 'R6: Star Forge is not available' }
  const need = unitType === 'fighter' ? 2 : 1
  const sysId = starForgeTarget(state, seat, unitType)
  if (!sysId) return { ok: false, error: 'R6: Star Forge cannot be completely resolved' }
  const sys = state.systems[sysId]
  const units: Unit[] = Array.from({ length: need }, (_, i) => ({ id: state.nextUnitId + i, type: unitType, owner: seat, damaged: false }))
  const players = [...state.players] as GameState['players']
  ...
  players[seat] = {
    ...
    reinforcements: { ...me.reinforcements, [unitType]: me.reinforcements[unitType] - need },
  }
  return {
    ok: true,
    value: {
      ...state,
      systems: { ...state.systems, [sysId]: { ...sys, space: [...sys.space, ...units] } },
      players,
      nextUnitId: state.nextUnitId + need,
      turnDone: true,
      log: [...state.log, { t: 'info', text: `seat ${seat} uses Star Forge to place ${need} ${unitType} in ${sysId}` }],
    },
  }
}
