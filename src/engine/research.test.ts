...
describe('R5 technology', () => {
  it('has 16 general techs, 7 unit upgrades (no PDS II, no War Sun) and faction techs', () => {
    expect(TECHS.filter(t => t.kind === 'general')).toHaveLength(16)
    expect(TECHS.filter(t => t.kind === 'upgrade').map(t => t.id).sort()).toEqual(['carrier_ii', 'cruiser_ii', 'destroyer_ii', 'dreadnought_ii', 'fighter_ii', 'infantry_ii', 'space_dock_ii'])
    expect(TECHS.filter(t => t.kind === 'faction').map(t => t.id).sort()).toEqual(['inheritance_systems', 'l4_disruptors', 'non_euclidean_shielding', 'super_dreadnought_ii', 'valefar_assimilator_x', 'valefar_assimilator_y'])
