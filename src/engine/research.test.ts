...
describe('R5 technology', () => {
  it('has 16 general techs, 7 generic unit upgrades plus 3 faction-named ones (no PDS II, no War Sun), and faction techs', () => {
    ...
    expect(TECHS.filter(t => t.kind === 'faction').map(t => t.id).sort()).toEqual(['inheritance_systems', 'l4_disruptors', 'non_euclidean_shielding', 'production_biomes', 'super_dreadnought_ii', 'valefar_assimilator_x', 'valefar_assimilator_y'])
