...
const producing = (seed = 1) => withTactical(toActionPhase(seed), { systemId: 'home-0', step: 'production' })
...
const fighters = (state: GameState) => state.systems['home-0'].space.filter(u => u.owner === 0 && u.type === 'fighter').length

describe('R4.4 production', () => {
  it('R4.4: produces ships into the space and infantry onto the dock planet, then finishes the step', () => {
    const s = producing()
    const r = produce(s, { cruiser: 1, infantry: 2 }, ['0.0.0'])
    if (!r.ok) throw new Error(r.error)
    expect(r.value.systems['home-0'].space.filter(u => u.owner === 0 && u.type === 'cruiser')).toHaveLength(1)
    expect(r.value.systems['home-0'].planets[0].ground.filter(u => u.owner === 0)).toHaveLength(7)
    expect(r.value.systems['home-0'].planets[0].exhausted).toBe(true)
    ...
    const ids = r.value.systems['home-0'].space.map(u => u.id)
    expect(new Set(ids).size).toBe(ids.length)
  })
  it('R4.4: the production limit is the planet resources plus the dock bonus', () => {
    const s = producing()
    expect(produce(s, { infantry: 8 }, ['0.0.0']).ok).toBe(false)     // limit 7
    expect(produce(s, { infantry: 6, fighter: 1 }, ['0.0.0']).ok).toBe(true)
  })
  it('R4.4: fighters and infantry come in pairs and Sarween Tools takes one off the total', () => {
    ...
    expect(produce(s, { fighter: 2, infantry: 2 }, ['0.0.0']).ok).toBe(true)
    expect(produce(withTechs(s, 0, ['sarween_tools']), { infantry: 2 }, []).ok).toBe(true)   // cost 1 minus 1
  })
  it('R4.4: producing a War Sun requires the War Sun technology (or Muaat)', () => {
    const rich = withPlayer(producing(), 0, { tradeGoods: 20 })
    // L1Z1X without war_sun tech fails
    expect(produce(rich, { warsun: 1 }, [], 12).ok).toBe(false)
    // L1Z1X with war_sun tech succeeds
    const teched = withTechs(rich, 0, ['war_sun'])
    const r = produce(teched, { warsun: 1 }, [], 12)
    if (!r.ok) throw new Error(r.error)
    expect(r.value.systems['home-0'].space.filter(u => u.owner === 0 && u.type === 'warsun')).toHaveLength(1)
  ...
  it('R4.4: only one flagship may exist at a time', () => {
    ...
    const hasFlagship = withUnits(rich, 'home-0', 0, ['flagship'])
  ...
  it('R4.4: a new ship\'s own capacity is pooled into the trim, so it can keep fighters that would otherwise be cut', () => {
    const sixFighters = withUnits(producing(), 'home-0', 0, ['fighter', 'fighter', 'fighter'])   // 3 existing + 3 more = 6, exactly capacity 6
    expect(fighters(sixFighters)).toBe(6)
    const r = produce(sixFighters, { carrier: 1, fighter: 4 }, ['0.0.0'])
    if (!r.ok) throw new Error(r.error)
  ...
  it('R4.4: a new ship\'s capacity, not just the pre-existing fleet, sets the exact fighter trim', () => {
    const s1 = withTactical(toActionPhase(1, 1), { systemId: 'home-1', step: 'production' })
    const packed = withUnits(s1, 'home-1', 1, ['fighter', 'fighter', 'fighter', 'fighter'])   // 1 existing + 4 more = 5, exactly fills capacity 5 (dreadnought 1 + carrier 4)
    const rich = withPlayer(packed, 1, { tradeGoods: 10 })
    const fightersS = (state: GameState) => state.systems['home-1'].space.filter(u => u.owner === 1 && u.type === 'fighter').length
  ...
  it('R4.4: the dock\'s free slots admit fighters even when carried infantry already fill the ship capacity', () => {
    const s = producing()
    const dockOnly = s.systems['home-0'].space.filter(u => u.owner !== 0 || u.type === 'dreadnought')   // drop the carrier and starting fighters, keep the capacity-2 dreadnought
    const packed: GameState = { ...s, systems: { ...s.systems, 'home-0': { ...s.systems['home-0'], space: dockOnly } } }
    const withInfantry = withUnits(packed, 'home-0', 0, ['infantry', 'infantry'])   // 2 infantry aboard, exactly filling the dreadnought's capacity of 2
    const withDockII = withTechs(withInfantry, 0, ['space_dock_ii'])
    const r = produce(withDockII, { fighter: 2 }, ['0.0.0'])
    if (!r.ok) throw new Error(r.error)
  ...
  it('R4.4: the reinforcement check runs on the trimmed order, so short fighters trim instead of rejecting', () => {
    ...
    const rich = withPlayer(withUnits(producing(), 'home-0', 0, ['fighter', 'fighter', 'fighter']), 0, { tradeGoods: 10 })
  ...
  it('R4.4: PDS and space docks cannot be produced in the duel', () => {
    const s = producing()
    expect(produce(s, { pds: 1 }, ['0.0.0']).ok).toBe(false)
    expect(produce(s, { spacedock: 1 }, ['0.0.0']).ok).toBe(false)
  })
  it('R7: what a production costs adds to the resources spent this round', () => {
    const first = produce(producing(), { infantry: 2 }, ['0.0.0'])
    if (!first.ok) throw new Error(first.error)
    expect(first.value.players[0].resourcesSpentThisRound).toBe(1)
    const again = withPlayer(withTactical(first.value, { systemId: 'home-0', step: 'production' }), 0, { tradeGoods: 6 })
    ...
    expect(second.value.systems['home-0'].space.filter(u => u.owner === 0 && u.type === 'dreadnought')).toHaveLength(2)
    expect(second.value.systems['home-0'].planets[0].ground.filter(u => u.owner === 0)).toHaveLength(11)
  })
  it('R4.4: production needs a space dock of your own in the active system and the production step', () => {
    const s = producing()
    expect(produce(withTactical(s, { systemId: 'bereg', step: 'production' }), { infantry: 2 }, ['0.0.0']).ok).toBe(false)
    expect(produce(withTactical(s, { systemId: 'home-0', step: 'movement' }), { infantry: 2 }, ['0.0.0']).ok).toBe(false)
  })
  describe('R14.1 blockade', () => {
    it('a dock with an enemy fleet and none of the seat\'s own ships cannot produce ships, but can still produce ground forces', () => {
      ...
      const emptied = deepFreeze({ ...base, systems: { ...base.systems, 'home-0': { ...base.systems['home-0'], space: [] } } })
      const s = withUnits(emptied, 'home-0', 1, ['destroyer'])
      expect(produce(s, { cruiser: 1 }, ['0.0.0']).ok).toBe(false)
      expect(produce(s, { infantry: 2 }, ['0.0.0']).ok).toBe(true)
    })
    it('is not blockaded when the seat also has a ship of their own in the system', () => {
      const s = withUnits(producing(), 'home-0', 1, ['destroyer'])   // seat 0's starting fleet is already there
      expect(produce(s, { cruiser: 1 }, ['0.0.0']).ok).toBe(true)
    })
