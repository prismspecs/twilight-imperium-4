...
- A white screen with no error in `debug.log` is usually not the app: the dev server's module graph can wedge after many hot-reloads (the page loads, but modules are served empty). Restart `npm run dev` (delete `node_modules/.vite` if it persists), then reload the tab.
