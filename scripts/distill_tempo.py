...
def normalize_tech(raw):
    ...
    # First try direct alias map
    result = TECH_ALIAS_MAP.get(cleaned)
    if result: return result
    # Then try common short codes (from AsyncTI4's tech codes)
    short_map = {
        'gd': 'gravity_drive', 'amd': 'antimass_deflectors', 'fl': 'fleet_logistics',
        'lwd': 'light_wave_deflector', 'ps': 'plasma_scoring', 'mdg': 'magen_defense_grid',
        'da': 'duranium_armor', 'ac': 'assault_cannon', 'nm': 'neural_motivator',
        'da': 'dacxive_animators', 'hm': 'hyper_metabolism', 'x89c4': 'x89_bacterial_weapon',
        'st': 'sarween_tools', 'tls': 'graviton_laser_system', 'td': 'transit_diodes',
        'ie': 'integrated_economy', 'inf2': 'infantry_ii', 'ff2': 'fighter_ii',
        'dd2': 'destroyer_ii', 'cv2': 'cruiser_ii', 'cn2': 'carrier_ii',
        'dn2': 'dreadnought_ii', 'pds2': 'space_dock_ii', 'ffp2': 'space_dock_ii',
        'dt2': 'space_dock_ii', 'is': 'inheritance_systems', 'sdn2': 'super_dreadnought_ii',
        'l4d': 'l4_disruptors', 'nes': 'non_euclidean_shielding',
        'tf-pillage': 'tf_pillage', 'tf-fabrication': 'tf_fabrication',
        'tf-mc': 'tf_mc',
    }
    return short_map.get(cleaned)

def normalize_faction(raw):
    # First try short codes directly (they're used in the NDJSON)
    if raw in FACTION_MAP.values():
        return raw
    # Then try full names
...
def main():
        ...
        with open(f, 'r', encoding='utf-8', errors='replace') as fp:
            for line in fp:
                ...
                if r.get('archetype') == 'TECH_RESEARCHED':
                    # techId is the field name in NDJSON (not 'tech')
                    tech = normalize_tech(r.get('payload', {}).get('techId'))
                    if tech: pd['techs'].append(tech)
                elif r.get('archetype') == 'SC_PICKED' and r.get('round') == 1:
                    # scNumber is the field name (1-8), strategyCard is not used
                    sc_num = r.get('payload', {}).get('scNumber')
                    # Map 1-8 to card IDs
                    sc_map = {1: 'imperial', 2: 'leadership', 3: 'politics', 4: 'diplomacy',
                              5: 'warfare', 6: 'technology', 7: 'construction', 8: 'trade'}
                    if isinstance(sc_num, int):
                        sc = sc_map.get(sc_num)
    ...
    # Manually build valid TypeScript object literal
    def fmt_list(items):
        if not items:
            return '[]'
    ...
    tempo_lines = []
    tempo_lines.append('{')
    items = list(tempo_data.items())
    for i, (fid, data) in enumerate(items):
        tempo_lines.append(f'  {fid}: {{')
        tempo_lines.append(f'    gamesCount: {data["gamesCount"]},')
        tempo_lines.append(f'    gamesWon: {data["gamesWon"]},')
        tempo_lines.append(f'    techOrder: {{')
        tempo_lines.append(f'      first: {fmt_list(data["techOrder"]["first"])},')
        tempo_lines.append(f'      second: {fmt_list(data["techOrder"]["second"])},')
        tempo_lines.append(f'      third: {fmt_list(data["techOrder"]["third"])},')
        tempo_lines.append('    },')
        tempo_lines.append(f'    round1ScTop: {fmt_list(data["round1ScTop"])},')
        tempo_lines.append(f'    expansionRatio: {data["expansionRatio"]},')
        tempo_lines.append(f'    aggression: {data["aggression"]},')
        tempo_lines.append('    avgResearchRound: 0.0')
        tempo_lines.append('  }' + (',' if i < len(items) - 1 else ''))
    tempo_lines.append('}')
    tempo_ts = '\n'.join(tempo_lines)
...
export const TEMPORAL_DATA: Readonly<TempoData> = """ + tempo_ts + """ as const
